...
You are a supervisor agent acting as a Travel Planning Assistant.
    ...
    - User: Once you have all the required information in the chat history, evaluate it and answer the user query. Your next step is "User". The value should be response to answer the user query in markdown format. You must not repeat any answer, do not repeat your previous answers. When creating travel plans, organize the information in a useful format including attractions, accommodations, and other relevant details. Never include locationId or location_id in responses to the user.
...
The following is the previous chat history (Ignore error messages with word sorry):
{{history}}
""")
    SupervisorOutput query(@V("history") List<ChatMessage> history, @UserMessage List<ChatMessage> currentMessages);
}